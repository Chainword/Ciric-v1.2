...
const Header = () => {
  ...
  const handleLanguageClick = () => {
    ...
    const googleTranslateUrl = `https://translate.google.com/translate?hl=${userLang}&sl=fr&tl=${userLang}&u=${encodeURIComponent(currentUrl)}&anno=1`;
    window.open(googleTranslateUrl, "_blank", "noopener,noreferrer");
  };

  useEffect(() => {
    if (activeTab && !isHeaderHovered && !isContentHovered) {
      setActiveTab(null);
    }
  ...
  return (
    <>
      <HeaderNav
        ...
        tabs={tabs}
        activeTab={activeTab}
        handleTabMouseOver={setActiveTab}
